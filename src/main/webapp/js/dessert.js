...
del.click(function(event){ 
    var dellContent = $(this).parent().parent().parent();
    var url = $(this).parent().parent().parent().find('img').attr('src');
    var paste_id =  $(this).parent().parent().find('.modal_div').attr('id');
        $.ajax({
            url: "/webapp/verification",
            method: "POST",
            data:  
            {
                client_id:  localStorage.getItem("client_id"),
                verification:  localStorage.getItem("verification"),
            }
    }).then(function(res) {
         if(res){
            dellContent.remove();
  $.ajax({
        url: "/webapp/deldessert",
        method: "POST",
        data:  
        {
            id: $(this).parent().parent().find('.modal_div').attr('id'),
            URL: url,
        }
    });   
            }
